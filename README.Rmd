...
* `x` A `data.table` of parameter values. Each list item contains a vector of parameter values of a single parameter.
...
The original parameters will removed from the trafo result.
...
ps = ParamSetFlat$new(
  params = c(
    list(
      ParamFlag$new(id = "switch"),
      ParamInt$new(id = "z", lower = 1, upper = 5)),
    repeatParam(5, ParamReal$new(id = "x", lower = 0, upper = 1))
  ),
  trafo = trafoOnRepeatedParam(
    fun = function(x, dict, tags) {
      ...
      data.table(x = .mapply(scale, cbind(z = dict$z, x), list()))
