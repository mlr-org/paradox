...
tunetoken_to_ps.RangeTuneToken = function(tt, param, args = list(), ...) {
  ...
  # print("rangeTT")
  # print(tt$content)
  # FIXME: i guess this tests if the bounds of a range token are valid....
  #FIXME: it seems really bad code here, that we pass values like the "logscale"
  # into the test for the param, which are by design of wrong type / value!!
...
pslike_to_ps.ParamSet = function(pslike, call, param, usersupplied = TRUE) {
  ...
  #FIXME: i have no idea what this does, there also no comments
