...
- public schema, Domain, property, and value projections own nested list
  ...
  is a marker only at a schema default/init position and has no meaning on a
  general ParamUty stored value;
...
| Arbitrarily deep/cyclic presentation metadata is copied through R's general duplicators | Detached public built-in metadata is ordinary, acyclic, and bounded to 64 attributes/recursive frames and 65,536 nodes; closure and `DOTSXP` presentation nodes reject. Direct quantile and typed-list compatibility preserve shallow nested identity but bound the top-level spine to 64. | Ordinary names, dimensions, dimnames, classes, row names, factors, time-series/comment metadata, and custom list metadata remain supported. Setter-invalid raw spellings, exotic recursive graphs, and overlong attribute spines now fail cleanly instead of risking an unbounded pairlist walk or native-stack exhaustion. Semantic function values/callbacks and documented opaque leaves are unaffected. |
