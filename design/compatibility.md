...
| Each public table operation recognizes metadata independently, or trusts row labels/cache payload | All six ingresses share one exact data.frame/data.table classifier. Names/classes and allowed data.table cache carriers are ordinary; caches are discarded. Raw integer/character row names are count-only: ordinary compact `+/-n` is decoded and stable ALTREP pays one Length/no Elt. An exact top-shell ALTREP owns names/class before callback-capable observation and copies one Length/one Elt per column. | Ordinary tables, base lazy duplicates, and zero-column data.frames remain supported. Missing, S4, object, attributed, or row-count-mismatched metadata now rejects on row-consuming paths. Direct `trafo` and a no-edge Design dependency plan validate row-name structure without extra column observations. `Design$transpose()` returns one empty configuration per row of a zero-column data.frame; an empty unclassed list remains zero rows. Package tables/facades remain canonical ordinary objects. |
