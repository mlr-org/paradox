...
| R22 | release blocker / serialized gateway completeness | categorical Paradox-1 sampler bridge | fixed, focused serialized-object proof and package usage analysis green | both generated legacy sampler environments now capture the exact versioned `sample_truncated` callable required by their shared syntactic body; the exact categorical branch never invokes it, so this removes the `R CMD check` NOTE without adding current-path work or changing categorical behavior |
| R23 | analyzer-only relational false positive | optional Domain receipt workspace | fixed, exact GCC-analyzer TU and complete analyzer build green; 1,591 focused assertions pass | allocation and all later bounds/special accesses now branch on the same cached interpretation booleans; all four masks retain one-or-zero allocations and the exact prior packed byte count, while the impossible skipped-allocation/used-bounds analyzer path disappears without a runtime guard or vendor attribute |
...
| M17 | release compatibility | function-valued ObjectTuneToken Domain representation | fixed, exact strict GCC/Clang focused snapshot and complete `test_to_tune.R` green; release proof pending | The general defensive data-table finalizer correctly rejects a closure recursively embedded in arbitrary presentation metadata, but `paradox_snapshot_builtin_domain()` also sent the package-defined opaque `repr` attribute through that owner. The documented `to_tune(list(exp = exp, square = function(x) x^2))` path therefore failed during checked assignment even though the function is a valid opaque factor level. Domain admission already treats `repr` as an exact presentation identity. Its ordinary non-ALTREP/non-S4 carrier is now rooted from the bounded top-level Domain metadata snapshot, withheld while the strict finalizer owns every structural attribute and column, and restored unchanged afterward. Generic closure metadata still rejects. Focused regression covers exact `repr` identity, printing, checked assignment, search-space construction, and recovered transformation identity; a forged top-level ALTREP `repr` still fails before traversal. The mutable-worktree preflight is `.local/checks/release-tune-repr-preflight-20260731-r1`; completion and source-manifest SHA-256 values are `31a267cb2ca84a632a84e20f154d144c8be5c5408c04ab95c4008a4bc89bb06f` and `8d5c6970427dd7c87aae8f978c7ed07c837076e0dd0b6fc434fdf1bb9194364b`. |
