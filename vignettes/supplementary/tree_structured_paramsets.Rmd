...
```{r}
 pt = ParamSetTree$new(ns.id = "keras",
      ...
      ParamCategorical$new(id = "reg_type", values = c("l1", "l2")),
...
```{r}
  ps = recursiveParaFac(nr = 2,
      ...
      ParamCategorical$new(id = "activation_fun", values = c("sigmoid", "tanh", "linear")))
  ps$sample()
  list.par.val = ps$getRecursiveList()
  ps$sample(3L) 
``` 

```{r}
library(keras)
tex = keras_helper(input.shape = 256, output.shape = 10L, output.act = "softmax", loss = "mse", lr = 0.0025, list.par.val = list.par.val)
eval(parse(text = tex))
```
