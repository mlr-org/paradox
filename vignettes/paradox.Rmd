...
The individual parameters can be accessed through the **`$params`** slot. It is also possible to get information about all parameters in a vectorized fashion using mostly the same slots as for individual `Param`s (i.e. `$class`, `$values` etc.), see `?ParamSet` for details.
...
Internally, the dependencies are represented as **`Dependency`** object, which can be listed in the **`$deps`** slot. The dependencies can also be accessed through the **`$deps_on`** slot, which returns a `data.table` containing dependency information.
```{r}
ps$deps
ps$deps_on
```
...
```
```{r}
generate_design_grid(psSmall, param_resolutions = c(A = 1, B = 1, C = 2))
```
```{r, echo = FALSE}
# FIXME: remove 'A = 1' from above as soon as issue #178 is fixed
```
...
The **`SamplerHierachical`** sampler is an auxiliary sampler that combines many 1D-Samplers to get a combined distribution. Its name "hierarchical" implies that it is able to respect parameter dependencies: Parameters only get sampled when their dependencies are met.
...
The transformation is performed when calling the **`$transpose`** function of the `Design` object returned by a `Sampler` with the **`trafo`** param set to `TRUE`. The following, for example, creates a parameter that is exponentially distributed:
```{r}
