...
### Migrating serialized Paradox 1 objects

Paradox 2 can migrate a legacy parameter set even when it is hidden inside a
larger R6 object, closure, attribute, or ordinary container. The recursive
upgrader preserves the identities of both the containing object and every
legacy ParamSet-family R6 shell:

This ParamSet-family migration must be run under R >= 4.0. Paradox 1 R6 shells
contain active bindings, and R 3.6 provides no safe way to retrieve their
functions for inert authentication. On R 3.6, current Paradox 2 objects and
standalone legacy Domain/Condition conversion remain supported; load a legacy
model under R >= 4.0 once, migrate it, and then save the upgraded object for
continued use on R 3.6.

One narrower boundary applies on R 4.5: that release has no policy-compliant
way to inspect a reached promise, so recursive migration fails closed if the
containing graph encounters one. Use R 4.0--4.4 or R >= 4.6 for such a graph;
R 4.5 remains sufficient when the traversed graph is promise-free. Ordinary
factory-created callbacks may retain formal promises in their lexical frames
even when the argument was forced or the callback never uses it, so this
boundary can arise without an explicit `delayedAssign()`.

```{r, eval=FALSE}
model = readRDS("legacy-model.rds")
upgrade_paradox_object_graph(model)
# `model` and its nested ParamSets are now current objects.
```

The crawler remembers visited objects, so shared references and cycles in the
containing graph are safe. It searches ordinary containers, attributes,
environments, and closure state without invoking active bindings, serialized
methods, or forcing promises. It deliberately stops at the global environment
and package/namespace/search-path infrastructure.

`upgrade_paradox_object()` remains useful when the file contains one Paradox
object directly. It is a pure converter: it does not mutate the input, and it
also handles standalone built-in Domain and Condition objects.

```{r, eval=FALSE}
legacy = readRDS("legacy-param-set.rds")
current = upgrade_paradox_object(legacy)
```

Calling an unmigrated legacy ParamSet method reports an error with the recursive
upgrade command by default. Applications that deliberately want transparent
first-use migration can opt in:

```{r, eval=FALSE}
options(paradox.legacy_object_action = "upgrade")
model = readRDS("legacy-model.rds")
model$train()  # A reached legacy ParamSet is silently upgraded before use.
```

Private ParamSet state is now opaque, ParamSet-family inheritance is additive
only, and Domain/Condition kinds are closed. Package code needing a live view
should use the Paradox-owned `ParamSetShadow`. The maintained bbotk Codomain
and miesmuschel legacy Shadow bridges are registered by their owner packages;
load the package that owns such an object before upgrading it so its `.onLoad()`
hook can register the exact bridge. An unknown legacy subclass still fails
closed rather than executing serialized extension code.
...
The parameter schema should be considered immutable. The documented mutable
state is `$values`, `$deps`, `$tags`, `$constraint`, `$extra_trafo`, and the
`$assert_values` assignment-policy flag.
...
The returned table is detached: mutating it, including its list-column
elements, does not mutate the `ParamSet`. The same rule applies to `$params`,
`$deps`, and `$domains`; use documented setters such as `$values`, `$tags`,
`$deps`, or `$add_dep()` to change package state.
...
The `$values` field contains a named list that is checked against parameter
Domains and the ParamSet constraint by default. A value whose dependencies are
currently unsatisfied is nevertheless stored; it is *dormant* rather than
invalid, as described below. Setting `$assert_values = FALSE` selects unchecked
value assignment; structural validation still applies.
...
Type, range, special-value, and custom constraints are automatically checked,
including for dormant values:
...
Dependencies affect explicit point checking, filtered value reads, constraints,
and designs. With `check_strict = TRUE` (the default), `$check()`, `$test()`,
and `$assert()` do not accept a supplied parameter whose dependency is not met.
These functions validate the supplied point only: they do not fill it from the
ParamSet's stored `$values`.

Assignment is intentionally different. A valid value may be stored while its
dependencies are unsatisfied. Raw `$values` retains this dormant value;
`$get_values()` filters it by default, and a later parent change makes it
visible again:

```{r}
branch = ps(
  enabled = p_lgl(init = FALSE),
  detail = p_int(0, 10, depends = enabled == TRUE)
)
branch$values$detail = 4
branch$values                         # contains dormant 'detail'
branch$get_values()                   # contains only 'enabled'
branch$values$enabled = TRUE
branch$get_values()                   # 'detail' is active again
```

Consequently, `$check(branch$values)` is not guaranteed to succeed while a raw
store contains dormant entries. Use `$check(branch$get_values())` to validate
the currently active configuration. `get_values(remove_dependencies = FALSE)`
is the explicit raw-store view.

When a dependency parent is absent from the point or store being evaluated, its
recorded `default` participates in the condition. An explicit value takes
precedence; a parent without a default does not satisfy the dependency. This
rule is recursive: a default belonging to an inactive parent cannot activate a
descendant.

```{r}
with_default = ps(
  enabled = p_lgl(default = TRUE),
  detail = p_int(0, 10, depends = enabled == TRUE)
)
with_default$check(list(detail = 4))  # parent default makes 'detail' active
```

The same default-aware activity rule controls `presence = "all"` and
`"required"` and `get_values(check_required = TRUE)`. It also determines the
input to `$constraint`: a constraint receives only entries that are active in
the configuration being validated; defaults select activity but are not added
to the callback list. Collection constraints evaluate activity in the complete
translated collection namespace and then give child callbacks their active
unprefixed slice. A collection-level dependency filters collection-level
reads; directly reading a child applies only dependencies owned by that child.

Bulk dependency-table replacement can represent a cycle even though normal
authoring should avoid one. Any check, filtered read, or constraint evaluation
that encounters such a cycle reports it as an error instead of attempting to
derive a partial configuration.

Finally, sampling and grid designs respect dependencies as before. Those
operations work on complete generated rows, so the new absent-parent default
rule does not change their results.
...
The **`$deps`** field returns a detached `data.table` view of the current
dependencies.  Mutating that returned table does not modify the `ParamSet`.
Use `$add_dep()` to add a dependency, or assign a complete valid dependency
table back to `$deps`; assignment is validated before the ParamSet state is
replaced.
...
`SamplerUnif` and `generate_design_random()` use the same native uniform engine
over the complete `ParamSet` graph. `SamplerUnif$samplers` is descriptive
metadata; use `SamplerHierarchical` when custom child samplers should execute.
...
For example, consider the `minsplit` parameter of the `classif.rpart` learner.
...
An empty `ParamSet`--not yet very useful--can be constructed with `ps()`:
...
typeof(search_space$levels$cost)
```
...
`"to_tune"` can also be constructed with a bounded, value-producing `Domain` object, i.e. something constructed with a suitable `p_***` call.
An unbounded `p_uty()` Domain or a zero-level `p_fct()` Domain is not a tuning range; use a bounded typed Domain or the BASE-`ParamSet` form when the resulting value is opaque.
...
```{r, eval = FALSE}
learner$param_set$values$type = "C-classification" # select classification mode explicitly
...
In Paradox 2 this Object TuneToken form accepts an ordinary BASE `ParamSet`, as
constructed by `ps(...)`; a `ParamSetCollection`, `ParamSetShadow`, or
third-party `ParamSet` subclass is not accepted as its content. The candidate is
snapshotted before its transformation runs, so callback mutation cannot race
the surrounding assignment or search-space conversion.
