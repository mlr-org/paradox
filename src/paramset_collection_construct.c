...
static int exact_child_class(SEXP self,
...
static int initialize_child(SEXP self, SEXP owner, SEXP roots,
  ...
  if (!exact_child_class(self, work_since_interrupt)) {
    return FALSE;
  }
  ...
  SET_VECTOR_ELT(
    roots,
    roots_offset + CONSTRUCTOR_ROOT_ENCLOSURE,
    enclosure
  );
...
static void set_table_attributes(SEXP table,
  ...
  static const char *const class_values[] = {"data.table", "data.frame"};
  SEXP classes = PROTECT(character_vector(class_values, 2));
  ...
  if (include_selfref) {
    ...
    Rf_setAttrib(table, R_NamesSymbol, R_NilValue);
    Rf_setAttrib(table, R_NamesSymbol, names);
  }
...
static SEXP new_table(const char *const *column_names,
    ...
    int include_row_names, int sorted_by_id, int include_selfref) {
...
static SEXP make_generated_tag(const char *prefix, size_t prefix_size,
  ...
  const size_t component_size = strlen(component_text);
...
static SEXP sorted_table(SEXP source, const char *const *column_names,
    const SEXPTYPE *types, R_xlen_t column_count, R_xlen_t row_count,
    int include_row_names, int include_selfref,
    R_xlen_t *work_since_interrupt) {
...
SEXP paradox_param_set_collection_construct(SEXP sets, SEXP tag_sets_sexp,
  ...
  int tag_sets = FALSE;
  int tag_params = FALSE;
  int postfix = FALSE;
  if (!exact_flag(tag_sets_sexp, &tag_sets) ||
      !exact_flag(tag_params_sexp, &tag_params) ||
      !exact_flag(postfix_sexp, &postfix)) {
    return R_NilValue;
  }

  SEXP set_names = R_NilValue;
  if (!validate_set_names(sets, &set_names, &work_since_interrupt)) {
    return R_NilValue;
  }
  PROTECT(set_names);
  ...
  for (R_xlen_t child_index = 0;
    ...
    if (!initialize_child(
        ...
        child,
        &work_since_interrupt
      ) || !checked_add(&total_params, child->param_count) ||
