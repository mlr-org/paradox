...
test_that("special_vals work for all ancestors of ParamSimple", {
  param_ancestors = list(ParamCategorical, ParamFlag, ParamInt, ParamReal)
  special_vals.list = list(
    list(1), 
    list('a'), 
    list(1:10), 
  ...
  for (param in param_ancestors) {
    for (special_vals in special_vals.list) {
