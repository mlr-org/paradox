...
test_that("vals", {
  ...
  ps$vals = list()
  expect_equal(ps$vals, named_list())
  ps$vals = list(d = 1, f = "a")
  expect_true(ps$check(list(d = 0, f = "a")))
  ps2 = ps$clone()
  ps2$subset(ids = c("d", "i"))
  expect_equal(ps2$vals, list(d = 1))
  ps2$vals = list(d = 0.5)
  expect_true(ps$check(list(d = 1, f = "a")))
  expect_equal(ps2$vals, list(d = 0.5))
  # check printer
  expect_output(print(ps2), "d.*<NoDefault>.*0.5")

  ps2 = ps$clone()
  ps2$subset(ids = c("i"))
  expect_equal(ps2$vals, set_names(list(), character(0)))
  ...
  ps2 = ps$clone()
  ps2$add(ps3)
  expect_equal(ps2$vals, list(d = 1, f = "a", x = 7))
...
test_that("vals calls assert", {
  ...
  expect_error(ps$vals <- list(xxx = 1), "not available")
  expect_error(ps$vals <- list(d = 9), "not <= 1")
})
...
test_that("required params are checked", {
  ...
  expect_error(ps$vals <- list(i = 2), "Missing required")
})
